chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
        chrome.runtime.sendMessage({action: 'give2'});
        console.log('give2');
    });
...
chrome.runtime.onMessage.addListener(
  function(request, sender, sendResponse) {
      ...
      console.log(xNum);
      //добавляет новый td в таблицу second.html
      function addTd(amount, id){
          var newtd = document.getElementById(id);
      ...
      function searchAndPaste(){
          for(var i = 0; i < xNum.length; i++){
              if(typeof xNum[i] !== 'undefined' && xNum[i] !== null){
                  ...
                  if(document.getElementById('date' + sDate)){ //существует ли такая дата в таблице
